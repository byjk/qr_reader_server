...
class AndroidCamera(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        # self._holder = AndroidWidgetHolder(size=[240,320], pos=self.pos)
...
class ZbarQrcodeDetector(AnchorLayout):
    ...
    qrdata = StringProperty('')
    serverIP = StringProperty('192.168.31.101')
    ...
    def __init__(self, **kwargs):
        ...
        osc.init()
        self._sound = SoundLoader.load('sound.wav')
    ...
    def _detect_qrcode_frame(self, instance, camera, data):
        ...
        while it.hasNext():
            ...
            if qrcode.data != self.qrdata:
                self.qrdata = qrcode.data
    ...
    @run_on_ui_thread
    def on_qrdata(self, instance, value):
        self._sound.play()
        self.sendMsg(value)
        if value.startswith('script:'):
            if self.btn1.text == '':
                self.btn1.text = value
            else:
                self.btn2.text = value
            

    def sendMsg(self, msg):
        if msg != '':
            osc.sendMsg('/qr_api', ['qr://'+msg, ], ipAddr=self.serverIP, port=serviceport)
...
BoxLayout:
    ...
    TextInput:
        text: detector.serverIP
        multiline: False
        size_hint_y: None
        height: '48dp'
        on_text: detector.serverIP = self.text
    ...
    BoxLayout:
        ...
        Button:
            id: btn1
            text: ''
            on_release: detector.sendMsg(self.text)
        Button:
            id: btn2
            text: ''
            on_release: detector.sendMsg(self.text)
